...
try {
    $stmt = $pdo->query("SELECT user_id AS moderator_id, prenom, nom, email FROM users WHERE role = 'moderator'");
    ...
    http_response_code(500);
    echo json_encode(['success' => false, 'message' => 'Erreur de base de données lors de la récupération des modérateurs: ' . $e->getMessage()]);
}
?>
